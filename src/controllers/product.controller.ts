...
export class ProductController {
    ...
    async create({body, file}: CreateProductBody, response: Response): Promise<Response<CreatedCategoryDto>>{
        const product = await this.productService.create({...body, image: file!.filename});
        return response.status(HttpStatus.CREATED).json(product);    
    }

    async getAll(_request: Request, response: Response): Promise<Response<CreatedCategoryDto[]>>{
        const products = await this.productService.getAll();
        return response.status(HttpStatus.OK).json(products);
    }

    async show({params}: Request, response: Response): Promise<Response<CreatedCategoryDto[]>>{
    ...
    async update({body: category, params }: Request, response: Response) {
        await this.productService.update(params.id, category?.name);
